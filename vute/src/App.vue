  ...
  methods: {
    ...
    changeDestinationPhoto() {
      ...
      this.currentDestinationIndex =
        (this.currentDestinationIndex + 1) % this.destinations.length;
    },
    startPlanning() {
      const params = new URLSearchParams({
        source: "desktop_btn_header",
      ...
      if (!newWindow || newWindow.closed || typeof newWindow.closed === "undefined") {
        ...
        const shouldRedirect = confirm(
          "Похоже, ваш браузер заблокировал открытие новой вкладки. Хотите перейти вручную?"
        );
        if (shouldRedirect) {
    ...
    learnMore() {
      this.showNotification(
        "Открываем подробную информацию о возможностях TravelGenius..."
      );
    },
